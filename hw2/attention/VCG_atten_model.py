...
class Video_Caption_Generator():
    ...
    def build_model(self):
        ...
        for i in range(0, self.n_video_lstm_step):
            ...
            # Store hidden state output during encoding stage
            if i == 0:
                h_prev = output2
            else:
                h_prev = tf.concat([h_prev, output2], 2) # b x h x n

        h_prev = tf.reshape(h_prev, [-1, self.n_video_lstm_step]) # (b x h) x n
        attention = tf.nn.xw_plus_b(h_prev, self.attention_W, self.attention_b) # (b x h) x n
